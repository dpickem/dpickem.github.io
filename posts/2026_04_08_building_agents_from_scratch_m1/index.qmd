---
title: "Building Agents from Scratch — M1: Foundation"
author: "Daniel Pickem"
date: "2026-04-08"
categories: [AI, Agents, Series]
image: series_announcement_linkedin_m1.png
lightbox: true
---

:::: {.callout-tip appearance="minimal"}
## 📝 LLM-Assisted Writing Disclosure

*This post was co-written with my AI assistant to a greater degree than my previous posts. There is a lot of ground to cover in this milestone, and I am still finding the right balance between publishing quickly and preserving the full authenticity of my own voice.*
::::
...
Before describing what M1 builds, it helps to pin down what "agent" actually means — because the term is used loosely enough to cover everything from a chatbot to a fully autonomous coding system. NVIDIA's [glossary entry on autonomous AI agents](https://www.nvidia.com/en-us/glossary/ai-agents/) offers a useful working definition (and is overall a great introductory read): an autonomous agent is an AI system that **reasons, plans, and executes multi-step tasks** based on a goal, built with security, privacy, and policy controls. The key distinction from a plain chatbot is the loop: the agent observes its environment, decides what to do next, acts, and then feeds the result back into its own context for the next decision (similar to the [sense-plan-act stack](https://en.wikipedia.org/wiki/Sense_Plan_Act) in robotics).
...
Coding agents are one important subtype of this broader pattern. They inherit the same anatomy above, but apply it to software development tasks and add code-specific infrastructure. Sebastian Raschka's [Components of a Coding Agent](https://magazine.sebastianraschka.com/p/components-of-a-coding-agent) provides a practical decomposition of that specialized harness and contains six core building blocks:
...
This project is based on the main design file in [design.md](https://github.com/dpickem/nemoclaw_escapades/blob/main/docs/design.md). Before implementing a milestone, though, I create a milestone-specific design file that contains sufficient details for implementation. Based on this sub-design document and the actual implementation, I write the corresponding blog post. So the chain of events that unfolds is the following: design.md -> design_m1.md -> code -> m1_setting_up_nemoclaw.md
...
[OpenShell deep dive](https://github.com/dpickem/nemoclaw_escapades/blob/main/docs/deep_dives/openshell_deep_dive.md) section on
remote hosting.
...
See [Lesson #18](#gateway-lifecycle-stopped--destroyed-but-start-doesnt-restart)
...
[Lesson #10](#type-nvidia-vs-type-openai-for-the-inference-provider)
...
[Lesson #19](#why-http-credentials-are-sufficient-for-slack-socket-mode)
for how.
...
[Lesson #19](#why-http-credentials-are-sufficient-for-slack-socket-mode) for
...
All three funnel into the same pipeline: filter → normalize → thinking → orchestrate → reply. The thin closures only differ in how they extract a `NormalizedRequest` from the Slack payload.
...
The orchestrator calls `self._backend.complete(request)` and receives an `InferenceResponse`. It never knows which provider is behind the call — the `BackendBase` interface isolates that entirely. The full backend implementation, including retry logic, error categorization, and the local-vs-sandbox credential split, is covered in the [Inference Backend](#inference-backend) section below.
...
## 10. `--type nvidia` vs `--type openai` for the inference provider {#type-nvidia-vs-type-openai-for-the-inference-provider}
...
| Inference routing (`openshell inference set`) | Yes | Yes |
...
[Lesson #8](#pythons-connect-tunneling-vs-openshells-rest-proxy-mode)).

## 11. The inference proxy injects the API key — the app must NOT send one {#the-inference-proxy-injects-the-api-key-the-app-must-not-send-one}
...
## 18. Gateway lifecycle: stopped ≠ destroyed, but `start` doesn't restart {#gateway-lifecycle-stopped--destroyed-but-start-doesnt-restart}
...
## 19. Why HTTP credentials are sufficient for Slack Socket Mode {#why-http-credentials-are-sufficient-for-slack-socket-mode}
...
2. **WebSocket connection** (`slack_websocket` policy) — the HTTP response
   ...
   killing the long-lived connection.
...
- [`docs/design.md`](https://github.com/dpickem/nemoclaw_escapades/blob/main/docs/design.md) — system shape, goals, and constraints
- [`docs/deep_dives/hermes_deep_dive.md`](https://github.com/dpickem/nemoclaw_escapades/blob/main/docs/deep_dives/hermes_deep_dive.md) — architecture notes on Hermes
- [`docs/deep_dives/openclaw_deep_dive.md`](https://github.com/dpickem/nemoclaw_escapades/blob/main/docs/deep_dives/openclaw_deep_dive.md) — architecture notes on OpenClaw
- [`docs/deep_dives/openshell_deep_dive.md`](https://github.com/dpickem/nemoclaw_escapades/blob/main/docs/deep_dives/openshell_deep_dive.md) — architecture notes on OpenShell
- [`docs/deep_dives/nemoclaw_deep_dive.md`](https://github.com/dpickem/nemoclaw_escapades/blob/main/docs/deep_dives/nemoclaw_deep_dive.md) — architecture notes on NemoClaw
